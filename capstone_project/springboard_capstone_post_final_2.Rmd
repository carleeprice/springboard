...
fullset$price_amount[which(fullset$status == "closed")] <- 0 sr
...
The building blocks of this inference are the funding total, and the return applied to that amount.  Let's have a closer look at funding totals between our two groups.  
...
Let's use this model instead to populate returns for our guessA group.

```{r}
guessA$inferred_return <- model5$coefficients[1] + model5$coefficients[2]*guessA$age
...
So these companies as a whole, over the seven years noted, returned 77.3% of the capital that was invested in them.  Keeping in mind that those that were funded later may not yet be sufficiently mature to have generated any return, we can look year-by-year.
...
Cluster 2 contains the greatest number of observations at 203.  This is the group that raised a very small amount of money relative to the others, but exited quickly and generated excellent returns.  
