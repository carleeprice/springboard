...
ggplot(companies3, aes(x = first_funding_at, y = funding_total_usd, col = status, alpha = 0.1)) + geom_jitter(alpha = 0.35) + scale_y_log10() + scale_x_discrete(breaks = c("2007/01/01", "2009/01/01", "2011/01/01", "2013/01/01", "2014/08/15")) + theme (axis.text.x = element_text(angle = 90, hjust = 1))
```
...
nrow(rounds2)
#check that all the companies thate were in rounds2 are still in rounds3, that only duplicates have been struck
which(!(as.vector(rounds3$company_name) %in% as.vector(rounds2$company_name)))
```
...
acquired$annual_return <- ((1+ acquired$return) ^ (365.25/as.integer(acquired$age_at_acquisition))) - 1
...
The relationship is positive (founded later = higher return) and significant at the 5% threshold.  We can visualise the relationship. Note the outlier, nPulse Technologies.  Acquired for 23X funding amount within a year.  Deserves some extra scrutiny.
...
ggplot(fullset, aes(status, funding_total_usd)) + geom_boxplot(colour = "#3366FF") +
  geom_jitter(width = 0.1, colour = "blue", alpha = 0.2) +
...
This model tells us that companies improve their probability of being acquired when they raise $2.4Bn or more. WHich none of these companies did.  This clearly needs more study.

And then WITHIN outcomes, so for companies specifically that were acquired, did those that received more funding generate higher returns for stakeholders?
...
This tells a slightly different story.  Among those that were acquired, companies are most likely to appear generate "bananas" returns if they've raised less money.  Those companies that have raised the most are in the lower-tier returns buckets.  This makes logical sense: the more money that goes into a company, the more they must realise on exit if they want the overall return to be high.  There are natural limits to the amount of money an exit can generate, and the probability at those extreme valuations of an acquisition starts to fall.  IPOs are of course available and will be studied separately. 

```{r}
#companies with "closed" status which raised a lot of money
```
...
Your company will more likely be acquired if you've raised gobs of money, but the resulting returns are likely to be low.
...
This gives us a sense of the magnitude of effect including our incomplete cases in the analysis will have.  Companies that were acquired for disclosed amounts generated as a group 8X the money invested in them.  But if we compared this "returned capital" to the total amount raised by all companies in the set, the numbers are less interesting.  Only 24.1% of the seed money invested was returned with certainty. To get a full picture however we need to consider companies that were acquired (cash generated for investors) and those that may simply be too early in their development to have returned capital yet.

Let's build some assumptions for the companies in our dataset that have not been acquired at a disclosed price, but have value just the same.

We'll need to add an age column for the remaining companies, as we did with our acquireds.  For those that haven't been acquired, and remain ongoing (and for the unknowns), this will be the end of the study period.  For those that are closed, we assume a final operating date that is a full year after their last fundraise.  This column will be used to transform absolute returns into annual rates.  
...
Clean this up a bit and look at the age structure for our groups.
...
This jibes with industry knowledge.  Companies that close tend to do so quickly (here, within a year and a half of their initial fundraise), acquisitions are happening a little bit faster than generally understood to be the case (<3 years here compared to expectation that exits take 5 to 7 years).  Operators have the longest average tenure of all.

Now we'll separate out those companies that are acquired at undisclosed prices, as we did above with those that were acquired and disclosed.  We'll call this new table "guess". There are 339 companies in this group. 
...
Much of the modelling work done above described influences on "status", which is already known for this group.  Let's see if some other numeric fields in our table will tell us about price.  At the 1% significance level, companies with greater ages are acquired for higher prices.  Each additional day adds $117k to valuation, after 160 days.  Not bad.  
...
So age influences price in a meaningful way.  But is just one factor in acquisition price: r2 here is 0.14.  Older (which not surprisingly means more total funding) companies tend to recieve higher prices on acquisition, but there remains a wide margin around the best-fit line.  We'll find a way to incorporate enterprise age as we move to build valuation assumptions for our "guess" group, without relying on a simple linear model.

We'll take a sampling approach to building our assumed prices.  

First, create a checkpoint here where we can experiment with modelling and come back to our unadjusted sets to reset if necessary
...
Another important piece of the puzzle, another relationship we've uncovered is between acquisition price and funding total.  Let's have a closer look at funding totals between our two groups.  

```{r}
par(mfrow=c(2,2))
hist(guess$funding_total_usd, breaks = 500, xlim = c(0, 10000000))
hist(acquired$funding_total_usd, breaks = 500, xlim = c(0, 10000000))
par(mfrow=c(1,1)) 
```

Since the companies for which we'll have to infer our prices are quite concentrated at funding levels <$2Mm, we'll have to be very deliberate about how we infer returns at this level.  Let's look at the distribution of returns from our "acquired disclosed" dataset, under 2Mm total funding.  There are 28 of these. We'll start building our inferred returns with the 206 companies in our "guess" set that fall into the same funding bucket.

```{r}
length(which(acquired$funding_total_usd < 2000000))
#is it better to populate these based on a returns number
par(mfrow=c(2,2)) 
hist(acquired$annual_return[which(acquired$funding_total_usd < 2000000)], main = "Acquired Returns")
#or better to use a price number
hist(acquired$price_amount[which(acquired$funding_total_usd < 2000000)], breaks = 50, xlim = c(0,75000000), main = "Acquired Prices")
par(mfrow=c(1,1)) 
smallAcquired <- subset(acquired, acquired$funding_total_usd < 2000000)
smallReturns <- acquired$annual_return[which(acquired$funding_total_usd < 2000000)]
length(which(guess$funding_total_usd < 2000000))
```

Then we can assign returns randomly to our small group using sampling:

```{r}
#guessA are our companies with <$2Mm funding
guessA <- subset(guess, guess$funding_total_usd < 2000000)
guessA$inferred_return <- sample(smallReturns, size = nrow(guessA), replace = TRUE)
guessA$method <- "A"
```

And then for the remaining (>2Mm funding) groups we have to be careful again, the distribution has a very long right tail.  We lose some of the nuance if we fail to include these outliers, yet we also must be careful to apply high returns multipliers appropriately.  We do this by subsetting into age brackets.

In order to adjust for this, we need to break the returns into two subgroups: young companies (which are distributed toward higher returns) and old companies (lower returns).  By doing this we take into account enterprise age, which we know from the analysis above is important.

```{r}
#guessB are companies with >$2Mm funding and older than average
guessB <- subset(guess, guess$funding_total_usd >= 2000000 & guess$age >= mean(acquired$age_at_acquisition))
#guessC are companies with >$2Mm funding and younger than average
guessC <- subset(guess, guess$funding_total_usd >= 2000000 & guess$age < mean(acquired$age_at_acquisition))
(nrow(guessA) + nrow(guessB) + nrow(guessC) == nrow(guess))
bigOldReturns <- acquired$annual_return[which((acquired$age_at_acquisition >= mean(acquired$age_at_acquisition)) & (acquired$funding_total_usd >= 2000000))]
#and then we remove the max value in this vector, as it creates massive problems for us later
#and can fairly be dismissed as an outlier
#bigOldReturns <- bigOldReturns[!bigOldReturns %in% c(max(bigOldReturns))]
bigOldReturns <- bigOldReturns[which(bigOldReturns < quantile(bigOldReturns)[4])]
#and do the same with our YoungReturns
bigYoungReturns <- acquired$annual_return[which(acquired$age_at_acquisition < mean(acquired$age_at_acquisition) & (acquired$funding_total_usd >= 2000000))]
#bigYoungReturns <- bigYoungReturns[!bigYoungReturns %in% c(max(bigYoungReturns))]
bigYoungReturns <- bigYoungReturns[which(bigYoungReturns < quantile(bigYoungReturns)[4])]
#(length(smallReturns) + length(bigOldReturns) + length(bigYoungReturns)) == nrow(acquired)
guessB$inferred_return <- sample(bigOldReturns, size = nrow(guessB), replace = TRUE)
guessB$method <- "B"
guessC$inferred_return <- sample(bigYoungReturns, size = nrow(guessC), replace = TRUE)
guessC$method <- "C"
guess <- rbind(guessA, guessB, guessC)
```

Check out the inferred returns distribution see if it looks correct at the low end.

```{r}
par(mfrow=c(2,2)) 
hist(acquired$annual_return, breaks = 30, xlim = c(0,20))
hist(guess$inferred_return, breaks = 30, xlim = c(0,20))
par(mfrow=c(1,1)) 
```

And then we can use the inferred annual return alongside age to generate inferred prices for our group:
...
```{r}
#guess$method <- "inferred"
acquired$method <- "reported"
plot1 <- subset(guess, select = c("inferred_return", "inferred_price", "age", "method"))
plot2 <- subset(acquired, select = c("annual_return", "price_amount", "age_at_acquisition", "method"))
colnames(plot1) <- c("return", "price", "age", "method")
colnames(plot2) <- c("return", "price", "age", "method")
...
```{r}
sum(acquired$price_amount)/sum(acquired$raised_amount_usd)
sum(guess$inferred_price)/sum(guess$funding_total_usd)
```

This doesn't look right.  Using graphics to understand where our sampling process has broken down, it appears our GroupA is problematic.  Let's look closer at this group: collect into vectors the assumptions and the resulting assignments, and compare them.

```{r}
smallReturnsB <- guess$inferred_return[which(guess$method == "A")]
smallPrices <- acquired$price_amount[which(acquired$funding_total_usd < 2000000)]
smallPricesB <- guess$inferred_price[which(guess$funding_total_usd < 2000000)]
summary(smallReturns)
summary(smallReturnsB)
summary(smallPrices)
summary(smallPricesB)
```

The returns distributions for the group we've sampled from and the group we've inferred match, by design.  The prices are mismatched, with the guessA prices appearing too high.  This has to do with the fact that our "guess" group has in general raised less money than the acquired group.  Let's try modelling this instead. We'll revisit out model4 using just the small companies.

```{r}
model5 <- lm(annual_return ~ age_at_acquisition, data = smallAcquired)
summary(model5)
```

Interesting -- here we see a negative slope, companies effectively recieving lower prices the older they are.  Let's use this model instead to populate returns for our guessA group. We're going to arbitrarily reduce that intercept slightly, to account for the fact that undisclosed acquisitions are likely to be less favourable for investors (who like to publicise their wins).

```{r}
guessA$inferred_return <- (model5$coefficients[1] -1) + model5$coefficients[2]*guessA$age
#some of these acquired companies are so old the model has assigned them negative returns
#let's fix that, by assigning the minimum value from our smallAcquired returns
guessA$inferred_return[which(guessA$inferred_return < 0)] <- quantile(smallAcquired$annual_return)[1]
```

And then re-run the comparison.

```{r}
guess <- rbind(guessA, guessB, guessC)
guess$inferred_price <- guess$funding_total_usd * (exp((log(guess$inferred_return + 1)) * (as.integer(guess$age)/365.25)))
guess$method <- "inferred"
plot1 <- subset(guess, select = c("inferred_return", "inferred_price", "age", "method", "funding_total_usd"))
plot2 <- subset(acquired, select = c("annual_return", "price_amount", "age_at_acquisition", "method", "funding_total_usd"))
colnames(plot1) <- c("return", "price", "age", "method", "funding_total")
colnames(plot2) <- c("return", "price", "age", "method", "funding_total")
plot3 <- rbind(plot1, plot2)
ggplot(plot3, aes(price, fill = method)) + geom_density(alpha = 0.2) + scale_x_log10()  #THIS IS A GREAT CHART LEAVE IT IN
ggplot(plot3, aes(age, fill = method)) + geom_histogram(alpha = 0.5, aes(y = ..density..), position = 'identity', binwidth = 60)
ggplot(plot3, aes(return, fill = method)) + geom_density(alpha = 0.2) + scale_x_log10()  #THIS IS A GREAT CHART LEAVE IT IN
#ggplot(plot3, aes(funding_total, fill = method)) + geom_histogram(alpha = 0.5, aes(y = ..density..), binwidth = 20) 
```

LOOK AT THAT BEAUTIFUL CHART.  Price distribution for our guess group resembles closely that of our acquired group, yet is lower specifically at the right tail.  It is also bulkier on the left tail, which is very good considering the age (young) and total fundraising activities (lesser) in our guess group relative to our acquired group.  Hooray!  This set of inferred prices and returns we can be quite happy with.
...
Now that we have a reasonable set of inferences, let's recombine them with our "acquired disclosed" set and derive some insights for our total set of acquired companies.

We recombine our acquired and guess sets into a single table of "exits".  This requires us to align the columnar information from our two tables.
...
So these companies as a whole, over the seven years noted, returned 77.6% of the capital that was invested in them.  Keeping in mind that those that were funded later may not yet be sufficiently mature to have generated any return, we can look year-by-year.

For 2007, 2008 & 2009:

```{r}
exits07 <- subset(exits, first_funding_at <= '2007/12/31' & !is.na(acquired_at))
funding07 <- subset(fullset, first_funding_at <= '2007/12/31')
sum(exits07$price_amount)/sum(funding07$funding_total_usd)
exits08 <- subset(exits, first_funding_at <= '2008/12/31'& first_funding_at > '2007/12/31' & !is.na(acquired_at))
funding08 <- subset(fullset, first_funding_at <= '2008/12/31'& first_funding_at > '2007/12/31')
sum(exits08$price_amount)/sum(funding08$funding_total_usd)
exits09 <- subset(exits, first_funding_at <= '2009/12/31'& first_funding_at > '2008/12/31' & !is.na(acquired_at))
funding09 <- subset(fullset, first_funding_at <= '2009/12/31'& first_funding_at > '2008/12/31')
sum(exits09$price_amount)/sum(funding09$funding_total_usd)
```

This means that of the total amound invested in companies founded in 2007, 103% came back to investors via exits by 2014.  Let's see if we can visualise this.  On the x axis are our dates, from 2007 - 2014.  The y axis is log(dollars).  Black points show raises (investors put money into the company) and the red points are exits (investors recieve money back).

```{r}
#this chart needs some work
ggplot(funding07, aes(x=funded_at, y=raised_amount_usd)) + geom_point() + geom_point(aes (x = acquired_at, y = price_amount), col = "red") +   scale_y_continuous(trans = "log") + scale_x_discrete(breaks = c("2007-01-01", "2008-01-23", "2011-03-04", "2013-01-14"))
```
...
Most of the operating companies in this set remain younger than the average age at acquisition; some additional value remains to be captured as these companies mature and some are acquired.
...
Here you can see the effect of outliers on our clustering.  Reading the columns left to right, nearly all of our companies listed as closed or unknown are grouped in Cluster 2 and 4.  Clusters 1 and 3 are particularly small, containing just 4 and 8 observations respectively.  Cluster 1 is an unusually old and well-funded group (average age 2288 days, funding $1.1Bn).  

We can also see from this clustering that our "closed" companies appear in two clusters: 2 and 4.  Also in Cluster 2 are 278 of our companies listed as "operating".  Are these companies at greater risk for closure?  If we had access to subsequent years data (say 2015 - 17) we might be able to test this theory (see: FUTURE WORK, below).
...
Here again our well-funded companies are grouped together (Cluster 1) and generated the lowest annual returns.  Cluster 4 contains the greatest number of observations at 225.  This is the group that raised a very small amount of money relative to the others, but exited quickly and generated excellent returns.  Be like Cluster 4.

#### RESULTS

Based on the information in this dataset, and using assumptions for our missing values that feel quite reasonable, we can fairly conclude that returns in this space are quite low.  Rather than the 20% annual return espoused by industry organisations and some leaders, it appears that returns are negative to very slightly positive.  
...
Remember this dataset was captured at a point in time.  Today Crunchbase has been updated with more current/complete information.  230 companies now fit our criteria for "acquired disclosed".  Let's see if the characteristics resemble what we have been working with

```{r}
#load the data
check1 = read.csv("oldacqd.csv")
#here is the list of names that were not included in our first set.  Some of these we deliberately took out.
names <- check1$Acquired.Company.Name[which(!(as.vector(check1$Acquired.Company.Name) %in% as.vector(acquired$name)))]
```
